...
import { User, UserJSON, UserFirestoreData, convertFirestoreData } from '@/types/user'
...
export async function signUpUser(formData: SignupFormData, password: string) {
    ...
    try{
        ...
        try{
            ...
            console.error("Rollback failed. Orphaned Auth user:", userCredential.user.uid, rollbackError);
        }
        throw error;
    }
}
